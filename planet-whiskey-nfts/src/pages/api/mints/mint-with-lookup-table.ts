...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log('[REGULAR-MINT-PURCHASE] Recording regular NFT purchase:', {
      walletAddress,
      nftMintAddress,
      collectionMintAddress,
      transactionSignature
    ...
    try {
      ...
      console.log('[REGULAR-MINT-PURCHASE] ✅ Transaction verified on blockchain');
    } catch (txError) {
      console.error('[REGULAR-MINT-PURCHASE] Failed to verify transaction:', txError);
    ...
    if (existingTransaction) {
      console.log('[REGULAR-MINT-PURCHASE] ℹ️ Transaction already recorded');
    ...
    console.log('[REGULAR-MINT-PURCHASE] ✅ Purchase recorded successfully:', newPurchase._id);

    return res.status(200).json({
      success: true,
      message: 'Regular NFT purchase recorded successfully',
      purchaseId: newPurchase._id
    });

  } catch (error) {
    console.error('[REGULAR-MINT-PURCHASE] Error:', error);
    return res.status(500).json({
      error: 'Failed to record regular NFT purchase',
