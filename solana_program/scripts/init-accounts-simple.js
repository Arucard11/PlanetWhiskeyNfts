 ...
 * Uses the admin keypair directly
 */
...
async function main() {
    ...
    // Load IDL and patch it to include empty accounts array (required by Anchor)
    const idl = JSON.parse(fs.readFileSync('./target/idl/lendingprogram.json', 'utf-8'));
    // Anchor requires accounts property, even if empty
    if (!idl.accounts) {
        idl.accounts = [];
    }
    // Create program
